...
> **Docs:** [Agent-to-Agent over ZAP](https://zap-proto.dev/docs/protocols/a2a) · part of the [ZAP Protocol](https://zap-proto.io)
...
`zap-a2a` layers cross-vendor agent comm semantics over the [ZAP transport](https://github.com/zap-proto/spec). Post-quantum confidentiality, mutual authentication, and zero-copy parse come from the wire; this repo only adds the cross-vendor agent comm message shape.

## Status

**v0.1 — schema-first.** This repo currently ships:

- [`schema/zap_a2a.zap`](schema/zap_a2a.zap) — wire format spec in ZAP schema language

Reference implementations (Go, Rust, TS) land in v0.2 once `zap-proto/spec` provides cross-language codegen for the schema.

## Why

| Property | A2A-over-HTTP+JSON | `zap-a2a` |
|---|---|---|
...
By the [composability theorem](https://github.com/zap-proto/papers/tree/main/composability), `zap-a2a` inherits ZAP-base's PQ confidentiality and mutual auth automatically — no a2a-specific PQ analysis required.
